...
● To solve this question, we will use the \`ufo_sightings.csv\` and the

\`day_parts_map.csv\` datasets.

● Then, using the data given in the dataset, we will group the sightings based on

seasons of the year (i.e. winter, spring, summer, and fall)

● From there, we can create a time series analysis plot to visualize if there are any

patterns in the sightings concerning the season.

● From the \`day_parts_map.csv\` dataset we can also visualize if there is any

correlation between astronomical events (such as twilight, sunrise, sunset, etc.) and
