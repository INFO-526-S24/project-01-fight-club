...
*Handling the accuracy of the time zones*

-   The time zones in the `places` are first joined onto the `states` column in the `ufo_sightings` dataset.

-   From there, we can find the local times for each of the reported sightings.

-   We will also be rounding up or down the dates, depending on the times as this will help us merge the datasets with the `day_parts_map` dataset as well, using which we will infer if the astrononmical events have an effect on the sightings.
